...
async function main() {
  console.log('=== BẮT ĐẦU KIỂM TRA & SEED DỮ LIỆU ===')

  try {
    // Kiểm tra xem đã có dữ liệu phòng trong DB chưa
    const [roomCountRes] = await sql`SELECT COUNT(*)::int AS count FROM phong`
    const hasData = roomCountRes && roomCountRes.count > 0

    if (hasData) {
      console.log('Phát hiện dữ liệu phòng đã tồn tại. Chỉ tiến hành thêm/bổ sung tài sản (tai_san_phong) cho các phòng hiện có...')
      const rooms = await sql`SELECT id, ma_phong, suc_chua_toi_da FROM phong`
      
      for (const r of rooms) {
        // Kiểm tra xem phòng này đã có tài sản chưa
        const [assetCountRes] = await sql`SELECT COUNT(*)::int AS count FROM tai_san_phong WHERE phong_id = ${r.id}`
        
        if (assetCountRes.count === 0) {
          console.log(`- Thêm tài sản mặc định cho phòng ${r.ma_phong} (sức chứa: ${r.suc_chua_toi_da})`)
          const capacity = r.suc_chua_toi_da
          await sql`INSERT INTO tai_san_phong (id, phong_id, ten_tai_san, so_luong) VALUES (gen_random_uuid(), ${r.id}, 'Giường', ${capacity})`
          await sql`INSERT INTO tai_san_phong (id, phong_id, ten_tai_san, so_luong) VALUES (gen_random_uuid(), ${r.id}, 'Nệm', ${capacity})`
          await sql`INSERT INTO tai_san_phong (id, phong_id, ten_tai_san, so_luong) VALUES (gen_random_uuid(), ${r.id}, 'Tủ quần áo', ${capacity})`
          await sql`INSERT INTO tai_san_phong (id, phong_id, ten_tai_san, so_luong) VALUES (gen_random_uuid(), ${r.id}, 'Thẻ từ', ${capacity})`
        } else {
          console.log(`- Phòng ${r.ma_phong} đã có sẵn ${assetCountRes.count} tài sản, bỏ qua.`)
        }
      }
      console.log('Hoàn thành cập nhật tài sản cho các phòng hiện có.')
    } else {
      console.log('Không tìm thấy dữ liệu phòng. Tiến hành seed mới toàn bộ từ đầu...')
      ...
      console.log('4. Đang tạo người dùng hệ thống (Sale, Quản lý, Kế toán)...')
      const actors = [
      ...
      // 6.5. Seed tài sản phòng (tai_san_phong) mặc định cho mỗi phòng mới
      console.log('6.5. Đang tạo danh sách tài sản mẫu cho từng phòng mới...')
      const newRooms = [
        { id: p101.id, name: 'P101', capacity: 1 },
        { id: p102.id, name: 'P102', capacity: 4 },
        { id: p103.id, name: 'P103', capacity: 2 }
      ]
      for (const r of newRooms) {
        await sql`INSERT INTO tai_san_phong (id, phong_id, ten_tai_san, so_luong) VALUES (gen_random_uuid(), ${r.id}, 'Giường', ${r.capacity})`
        await sql`INSERT INTO tai_san_phong (id, phong_id, ten_tai_san, so_luong) VALUES (gen_random_uuid(), ${r.id}, 'Nệm', ${r.capacity})`
        await sql`INSERT INTO tai_san_phong (id, phong_id, ten_tai_san, so_luong) VALUES (gen_random_uuid(), ${r.id}, 'Tủ quần áo', ${r.capacity})`
        await sql`INSERT INTO tai_san_phong (id, phong_id, ten_tai_san, so_luong) VALUES (gen_random_uuid(), ${r.id}, 'Thẻ từ', ${r.capacity})`
      }
      console.log('Đã tạo tài sản mặc định cho các phòng mới.')
